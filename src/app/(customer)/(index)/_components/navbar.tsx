...
const defaultNavbarData: NavbarClientProps = {
  user: null,
  categories: [],
  site: {
    webname: "Shopverse",
...
export default function Navbar() {
  ...
  const [hasLoadedNavbarData, setHasLoadedNavbarData] = useState(false);
  const [loginModalOpen, setLoginModalOpen] = useState(false);

  const loadNavbarData = useCallback(async () => {
    try {
      const response = await fetch("/api/customer/navbar", {
        cache: "no-store",
        credentials: "include",
      });
      ...
      const data = (await response.json()) as NavbarClientProps;

      setNavbarData({
        user: data.user ?? null,
        categories: Array.isArray(data.categories) ? data.categories : [],
        site: data.site ?? defaultNavbarData.site,
      });
    } catch (error) {
      console.error("Failed to load navbar data", error);
    } finally {
      setHasLoadedNavbarData(true);
    }
  }, []);
  ...
  const { user, categories, site } = navbarData;
  const isLoadingAccount = !hasLoadedNavbarData;
  ...
  if (isLoading) {
    return (
      <div
        ...
        className="h-11 w-44 animate-pulse rounded-full bg-white/10"
      />
    );
  }
  ...
  return (
    <div className="relative">
      <button
        ...
        disabled={isLoading}
        onClick={() => setOpen((prev) => !prev)}
        className="group flex flex-col items-center justify-center text-gray-600 transition hover:-translate-y-0.5 hover:text-[#110843] disabled:cursor-wait disabled:opacity-60"
      >
        <User className="h-6 w-6 transition group-hover:scale-110" />
        <span className="mt-1 text-[11px] font-medium">
          {isLoading ? "..." : user ? "Account" : "Sign In"}
        </span>
      </button>
